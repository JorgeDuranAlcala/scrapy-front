    ...
    <Card sx={{ ...sx }}>
      <CardContent sx={{ gap: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
          <Typography variant={stats ? 'body2' : 'subtitle1'}
            fontSize={!stats ? 12 : undefined}
            fontWeight={!stats ? 600 : undefined}
          >
            {title}
          </Typography>
          <Typography variant='h6'>{stats}</Typography>
        </Box>
        <CustomAvatar skin='light' color={avatarColor} sx={{ borderRadius: "6px", width: avatarSize, height: avatarSize }}>
